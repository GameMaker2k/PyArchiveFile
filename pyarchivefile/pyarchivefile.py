...
'''
    ...
    $FileInfo: pyarchivefile.py - Last Update: 3/14/2026 Ver. 0.30.14 RC 1 - Author: cooldude2k $
'''
...
__use_ini_name__ = os.path.join(filecfgpath, "archivefile.ini")
__use_json_file__ = False
__use_json_name__ = os.path.join(filecfgpath, "archivefile.json")
...
__version_info__ = (0, 30, 14, "RC 1", 1)
__version_date_info__ = (2026, 3, 14, "RC 1", 1)
...
class MultiPartFile(io.RawIOBase):
    ...
    def __init__(self, parts, mode="r+b", part_size=1024*1024*1024,
                 create_pattern=None, start_index=0):
        ...
        if "w" in mode:
            if not self._files:
                if not create_pattern:
                    raise ValueError("Need initial part or create_pattern")
                self._ensure_part_exists(0)

            self._files[0].seek(0)
            self._files[0].truncate(0)

            for i in range(1, len(self._files)):
                self._maybe_close_part(i)
    ...
    def _add_part(self, p):
        if isinstance(p, (str, bytes, os.PathLike)):
            path = Path(p)
            fp = open(path, self._mode)
            owned = True
        ...
        size = self._get_size_fp(fp)

        self._files.append(fp)
        self._owned.append(owned)
        self._sizes.append(size)

    def _maybe_close_part(self, idx):
        if idx < len(self._files) and self._owned[idx]:
            try:
                self._files[idx].close()
            except:
                pass

    def _make_new_part_path(self):
        if not self._create_pattern:
            raise io.UnsupportedOperation("create_pattern required")

        path = Path(self._create_pattern.format(index=self._next_index))
        self._next_index += 1
        return path

    def _ensure_part_exists(self, idx):
        while len(self._files) <= idx:

            path = self._make_new_part_path()
    ...
    def readinto(self, b):
        ...
        while remaining > 0 and self._pos < self._total:
            ...
            n = min(remaining, self._sizes[idx] - off)

            data = f.read(n)
            if not data:
                break

            mv[out:out+len(data)] = data
    ...
    def _ensure_capacity(self, end_pos):

        if end_pos <= self._total:
            return

        if self._part_size is None:

            last = len(self._files) - 1
            grow = end_pos - self._total

            self._files[last].truncate(self._sizes[last] + grow)

            self._sizes[last] += grow
            self._recalc_total()
            return

        while self._total < end_pos:

            last = len(self._files) - 1
            cap = self._part_size - self._sizes[last]

            if cap <= 0:
                self._ensure_part_exists(len(self._files))
                continue

            grow = min(cap, end_pos - self._total)
    ...
    def write(self, b):
        ...
        while remaining > 0:
            ...
            idx, part_off = self._locate(self._pos)

            f = self._files[idx]
            f.seek(part_off)

            room = self._sizes[idx] - part_off

            n = min(remaining, room)

            w = f.write(mv[off:off+n])
...
def multiopen(parts, mode="r+b", buffering=io.DEFAULT_BUFFER_SIZE,
              part_size=1024*1024*1024, create_pattern=None, start_index=0):
    ...
    return io.BufferedRandom(raw, buffering)
...
def AppendFilesWithContentFromInFileToStackedOutFile(infiles, outfile, listtype="dir", fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, extradata=[], jsondata={}, checksumtype=["md5", "md5", "md5", "md5", "md5"], formatspecs=__file_format_multi_dict__, saltkey=None, verbose=False, returnfp=False):
    if not isinstance(infiles, list):
        infiles = [infiles]
    returnout = False
    for infileslist in infiles:
        returnout = AppendFilesWithContentFromTarFileToOutFile(infileslist, outfile, listtype, fmttype, compression, compresswholefile, compressionlevel, compressionuselist, extradata, jsondata, checksumtype, formatspecs, saltkey, verbose, True)
        if(not returnout):
            break
        else:
            outfile = returnout
...
def AppendFilesWithContentToList(infiles, fmttype="auto", dirlistfromtxt=False, extradata=[], jsondata={}, contentasfile=False, compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, followlink=False, checksumtype=["md5", "md5", "md5"], formatspecs=__file_format_dict__, saltkey=None, verbose=False):
    ...
    for curfname in GetDirList:
        ...
        if(not followlink and hasattr(os.path, "isjunction") and os.path.isjunction(fname)):
            ...
            ftype = 11
        elif(hasattr(stat, "S_ISSOCK") and stat.S_ISSOCK(fpremode)):
            ftype = 15
        else:
            ftype = 0
        flinkname = ""
...
if(libarchive_support):
    ...
    def _has_archive_class():
        A = getattr(libarchive, "Archive", None)
        return A if callable(A) else None
    def open_archive_reader(infile):
        """
        Returns a context manager that yields a libarchive reader (iterable of entries).

        Supports (depending on what this installed 'libarchive' package exposes):

        Preferred (if available):
          - libarchive.Archive(path_or_fd)

        Otherwise falls back to libarchive-c style:
        ...
        """
        Archive = _has_archive_class()
        # ---- 0) If Archive class exists, prefer it (works for path or fd int) ----
        if Archive is not None:
            # Path-like -> Archive(path)
            if _is_pathlike(infile):
                return Archive(infile)
            # FD int -> Archive(fd)
            if isinstance(infile, int):
                return Archive(infile)
            # File object -> Archive(fileno)
            if hasattr(infile, "read") and hasattr(infile, "fileno"):
                return Archive(infile)
            # BytesIO / bytes-like:
            # Archive usually doesn't accept raw bytes directly; fall through to memory/temp handling.
        # ---- 1) Path-like fallback ----
        if _is_pathlike(infile):
            return libarchive.file_reader(infile)
        # ---- 2) BytesIO / bytes-like -> memory_reader (or temp file fallback) ----
        data = None
            ...
            class _TempFileReaderCM:
                ...
                def __enter__(self):
                    ...
                    try:
                        ...
                        self._tmp.flush()
                        self._tmp.close()
                        # If Archive exists, prefer it even in fallback
                        Archive2 = _has_archive_class()
                        if Archive2 is not None:
                            self._cm = Archive2(self._tmp.name)
        ...
        # ---- 3) FD int fallback (libarchive-c style) ----
        if isinstance(infile, int):
            return libarchive.fd_reader(infile)
        # ---- 4) File object with fileno() fallback ----
        if hasattr(infile, "read") and hasattr(infile, "fileno"):
...
def AppendReadInFileWithContentToOutFile(infiles, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, extradata=[], jsondata={}, checksumtype=["md5", "md5", "md5", "md5", "md5"], formatspecs=__file_format_multi_dict__, insaltkey=None, outsaltkey=None, verbose=False, returnfp=False):
    return AppendFilesWithContentFromInFileToOutFile(infiles, outfile, "dir", fmttype, compression, compresswholefile, compressionlevel, compressionuselist, extradata, jsondata, checksumtype, formatspecs, saltkey, verbose, returnfp)
...
def AppendFilesWithContentFromTarFileToOutFile(infiles, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, extradata=[], jsondata={}, checksumtype=["md5", "md5", "md5", "md5", "md5"], formatspecs=__file_format_multi_dict__, saltkey=None, verbose=False, returnfp=False):
    return AppendFilesWithContentFromInFileToOutFile(infiles, outfile, "tar", fmttype, compression, compresswholefile, compressionlevel, compressionuselist, extradata, jsondata, checksumtype, formatspecs, saltkey, verbose, returnfp)
...
if(not libarchive_support):
    def AppendFilesWithContentFromBSDTarFileToOutFile(infiles, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, extradata=[], jsondata={}, checksumtype=["md5", "md5", "md5", "md5", "md5"], formatspecs=__file_format_multi_dict__, saltkey=None, verbose=False, returnfp=False):
        return False
else:
    def AppendFilesWithContentFromBSDTarFileToOutFile(infiles, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, extradata=[], jsondata={}, checksumtype=["md5", "md5", "md5", "md5", "md5"], formatspecs=__file_format_multi_dict__, saltkey=None, verbose=False, returnfp=False):
        return AppendFilesWithContentFromInFileToOutFile(infiles, outfile, "bsd", fmttype, compression, compresswholefile, compressionlevel, compressionuselist, extradata, jsondata, checksumtype, formatspecs, saltkey, verbose, returnfp)

def AppendFilesWithContentFromBSDTarFileToStackedOutFile(infiles, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, extradata=[], jsondata={}, checksumtype=["md5", "md5", "md5", "md5", "md5"], formatspecs=__file_format_multi_dict__, saltkey=None, verbose=False, returnfp=False):
    if not isinstance(infiles, list):
        infiles = [infiles]
    returnout = False
    for infileslist in infiles:
        returnout = AppendFilesWithContentFromBSDTarFileToOutFile(infileslist, outfile, fmttype, compression, compresswholefile, compressionlevel, compressionuselist, extradata, jsondata, checksumtype, formatspecs, saltkey, verbose, True)
        if(not returnout):
            break
        else:
            outfile = returnout
...
def AppendFilesWithContentFromZipFileToOutFile(infiles, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, extradata=[], jsondata={}, checksumtype=["md5", "md5", "md5", "md5", "md5"], formatspecs=__file_format_multi_dict__, saltkey=None, verbose=False, returnfp=False):
    return AppendFilesWithContentFromInFileToOutFile(infiles, outfile, "zip", fmttype, compression, compresswholefile, compressionlevel, compressionuselist, extradata, jsondata, checksumtype, formatspecs, saltkey, verbose, returnfp)
...
if(not rarfile_support):
    ...
    def AppendFilesWithContentFromRarFileToOutFile(infiles, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, extradata=[], jsondata={}, checksumtype=["md5", "md5", "md5", "md5", "md5"], formatspecs=__file_format_multi_dict__, saltkey=None, verbose=False, returnfp=False):
        return AppendFilesWithContentFromInFileToOutFile(infiles, outfile, "rar", fmttype, compression, compresswholefile, compressionlevel, compressionuselist, extradata, jsondata, checksumtype, formatspecs, saltkey, verbose, returnfp)
...
if(not py7zr_support):
    ...
    def AppendFilesWithContentFromSevenZipFileToOutFile(infiles, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, extradata=[], jsondata={}, checksumtype=["md5", "md5", "md5", "md5", "md5"], formatspecs=__file_format_multi_dict__, saltkey=None, verbose=False, returnfp=False):
        return AppendFilesWithContentFromInFileToOutFile(infiles, outfile, "7zip", fmttype, compression, compresswholefile, compressionlevel, compressionuselist, extradata, jsondata, checksumtype, formatspecs, saltkey, verbose, returnfp)
...
if(not libarchive_support):
    def PackArchiveFileFromBSDTarFile(infile, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, checksumtype=["md5", "md5", "md5", "md5", "md5"], extradata=[], jsondata={}, formatspecs=__file_format_dict__, saltkey=None, verbose=False, returnfp=False):
        return False
else:
    def PackArchiveFileFromBSDTarFile(infile, outfile, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, checksumtype=["md5", "md5", "md5", "md5", "md5"], extradata=[], jsondata={}, formatspecs=__file_format_dict__, saltkey=None, verbose=False, returnfp=False):
        return AppendFilesWithContentFromBSDTarFileToOutFile(infile, outfile, fmttype, compression, compresswholefile, compressionlevel, compressionuselist, extradata, jsondata, checksumtype, formatspecs, saltkey, verbose, returnfp)
...
def PackArchiveFileFromDirListAlt(infiles, outfile, dirlistfromtxt=False, fmttype="auto", compression="auto", compresswholefile=True, compressionlevel=None, compressionuselist=compressionlistalt, followlink=False, checksumtype=["md5", "md5", "md5", "md5", "md5"], extradata=[], jsondata={}, formatspecs=__file_format_multi_dict__, saltkey=None, verbose=False, returnfp=False):
...
def BSDTarFileListFile(infile, formatspecs=__file_format_multi_dict__, verbose=False, returnfp=False):
